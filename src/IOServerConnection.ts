...
import { inVCSocketTimeoutError } from './SocketTimeoutError';
		...
		return new Promise((resolve, reject) => {
			if (!this.socket) {
				...
				this.socket.timeout(3000).emit('request', socketMessage, (timeout, serverError, response) => {
					if (timeout) reject(new inVCSocketTimeoutError('Request timed out'));
	...
	public async request(request: SocketMessage): Promise<unknown> {
		...
		for (let tries = 0; tries < 3; tries++) {
			try {
				...
				if (error instanceof inVCSocketTimeoutError)
